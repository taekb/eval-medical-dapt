...
'''
    Older implementation of LLaVA models, directly taken from LLaVA-Med repository.

    Reference: https://github.com/microsoft/LLaVA-Med/blob/b9a98a736d2ef05bcf5ff345be6403fb3a664eaf/llava/model/llava.py
...
class LlavaLlamaModel(LlamaModel):
    ...
    def initialize_vision_modules(self, vision_tower, mm_vision_select_layer,
                                  pretrain_mm_mlp_adapter=None, tune_mm_mlp_adapter=False, dtype=torch.float16):

        if "BiomedCLIP" in vision_tower:
            self.vision_tower_name = "microsoft/BiomedCLIP-PubMedBERT_256-vit_base_patch16_224"
            return self.initialize_vision_modules_from_biomed_clip(vision_tower, mm_vision_select_layer,
                                pretrain_mm_mlp_adapter=None, tune_mm_mlp_adapter=False, dtype=dtype)
        else:
    ...
    def initialize_vision_modules_from_openai_clip(self, vision_tower, mm_vision_select_layer,
        ...
        if not hasattr(self, 'mm_projector'):
            self.mm_projector = nn.Linear(vision_config.hidden_size, self.config.hidden_size, dtype=dtype)
...
class LlavaLlamaForCausalLM(LlamaForCausalLM):
    ...
    def forward(
        ...
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        images: Optional[torch.FloatTensor] = None, # NOTE: Can be a list of image tensors
        return_dict: Optional[bool] = None,
    ...
    def initialize_vision_tokenizer(self, mm_use_im_start_end, tokenizer, device,
        ...
        patch_token_added = tokenizer.add_tokens([DEFAULT_IMAGE_PATCH_TOKEN], special_tokens=True)
        if patch_token_added:
            self.resize_token_embeddings(len(tokenizer))

        if mm_use_im_start_end:
            start_token_added = tokenizer.add_tokens([DEFAULT_IM_START_TOKEN])
            end_token_added = tokenizer.add_tokens([DEFAULT_IM_END_TOKEN])
            num_new_tokens = start_token_added + end_token_added
...
#AutoConfig.register("old-llava", LlavaConfig)
