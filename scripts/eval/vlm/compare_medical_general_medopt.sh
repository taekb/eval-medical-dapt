...
start_time=$(date +%s)
logdir="./vlm_logs/compare_medical_general_medopt"
mkdir -p "${logdir}"
...
./eval_few_shot_medopt.sh "llava-v0-7b" "${datasets}" "$1" "$2" 2>&1 | tee -a "${logdir}/llava-v0_${2}.log" 

# Med-Flamingo-9B and Open-Flamingo-9B
conda activate open-flamingo-env
./eval_few_shot_medopt.sh "open-flamingo-9b" "${datasets}" "$1" "$2" 2>&1 | tee -a "${logdir}/open-flamingo_${2}.log"
