...
# Llama-3-70B-Instruct vs. OpenBioLLM-70B
./eval_few_shot.sh "llama-3-70b" "${datasets}" "$1" "$2" "$3" 2>&1 | tee -a "${logdir}/llama-3-70b_${2}.log"
./eval_few_shot.sh "openbiollm-70b" "${datasets}" "$1" "$2" "$3" 2>&1 | tee -a "${logdir}/openbiollm-70b_${2}.log"

# Llama-3-8B-Instruct vs. OpenBioLLM-80B
...
# Mistral-7B-v0.1 vs. BioMistral-7B
